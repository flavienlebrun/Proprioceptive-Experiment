function [C_header,Data,String] = lireDataPosition2(filename)
...
C = textscan(fileID, '%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s','Delimiter',';');
...
FirstExperiment = C{17};
SecondExperiment = C{18};
TargetHold = C{19};
