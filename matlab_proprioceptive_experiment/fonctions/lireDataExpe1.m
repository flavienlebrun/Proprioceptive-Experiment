function [C_header,C_posCenter,Data] = lireDataExpe1(filename)
...
C = textscan(fileID, '%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s','Delimiter',';');
...
StartingPositionxstr =  strrep(C{18},',','.');
StartingPositionx = str2double(StartingPositionxstr);

StartingPositionystr =  strrep(C{19},',','.');
StartingPositiony = str2double(StartingPositionystr);

StartingPositionzstr =  strrep(C{20},',','.');
StartingPositionz = str2double(StartingPositionzstr);

CubeCenterxstr = strrep(C{21},',','.');
CubeCenterx = str2double(CubeCenterxstr);

CubeCenterystr = strrep(C{22},',','.');
CubeCentery = str2double(CubeCenterystr);

CubeCenterzstr = strrep(C{23},',','.');
CubeCenterz = str2double(CubeCenterzstr);

fclose(fileID);

% concatenation
Data = [ExperimentID,participantID,date,PosRx,PosRy,PosRz,QuaternionRotw,QuaternionRotx,QuaternionRoty,QuaternionRotz,targetNumber,condition,aim_number,time_elapsed,PosTargetx,PosTargety,PosTargetz,StartingPositionx,StartingPositiony,StartingPositionz,CubeCenterx,CubeCentery,CubeCenterz];
